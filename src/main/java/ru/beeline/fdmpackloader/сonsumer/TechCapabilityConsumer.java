...
@Component
@EnableRabbit
public class TechCapabilityConsumer {

    @Autowired
    PackageService packageService;
    private ObjectMapper objectMapper = new ObjectMapper();

    @RabbitListener(queues = "${queue.tech-capability.name}")
    public void processTechCapabilityQueue(String message) {
        log.info("Received from tech_capability_queue : " + message, new String(message.getBytes()));
        try {
            ...
            packageService.addTechCapability(
                    PackagePart.builder()
